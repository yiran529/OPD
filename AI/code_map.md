...
- `LinearOPSD/corruption.py`: trainer-time helpers for `linear_opsd` gold-prefix sampling, clean/mild/hard sampled-tail prefix construction, exact token-level teacher trace construction without inline sampled-tail markers, and generic token-sequence padding.
- `LinearOPSD/data_collator.py`: keeps original privileged-prompt collation for `conditioning_mode=opsd`; `linear_opsd` now only tokenizes static materials (`problem` prompt ids and `solution` ids), with a configurable student thinking-mode chat-template switch, and leaves corruption/prompt construction to trainer time.
- `LinearOPSD/opsd_trainer.py`: upstream trainer shell with original JSD/tinker paths intact; `conditioning_mode=linear_opsd` now samples a gold prefix, chooses clean/mild/hard prefix mode, optionally generates a sampled tail online with the current student, builds teacher prompts with a teacher thinking-mode switch and exact token-level trace tail matching, then reuses the shared-rollout generation and JSD supervision path for the recovery segment.
- `LinearOPSD/loss_detail_logging.py`: optional, default-off JSD loss diagnostics used by `opsd_trainer.py`; records high-loss rollout token events, position-loss quantiles/histograms, rollout-position buckets, and local JSONL/W&B tables without changing training loss or rollout behavior.
- `LinearOPSD/token_loss_categories.py`: token-category classifier for optional loss-detail summaries; groups actual sampled tokens and top JSD-contributor tokens into coarse categories such as discourse words, math symbols, numbers, formatting, finalization, math actions/objects, proof structure, special tokens, and ordinary words.
