...
- `LinearOPSD/corruption.py`: trainer-time helpers for `linear_opsd` gold-prefix sampling, clean/mild/hard sampled-tail prefix construction, teacher trace construction with a low-interference sampled-tail marker, and generic token-sequence padding.
...
- `LinearOPSD/opsd_trainer.py`: upstream trainer shell with original JSD/tinker paths intact; `conditioning_mode=linear_opsd` now samples a gold prefix, chooses clean/mild/hard prefix mode, optionally generates a sampled tail online with the current student, builds teacher prompts with a teacher thinking-mode switch, then reuses the shared-rollout generation and JSD supervision path for the recovery segment.
