...
def build_experiment_name(model_name: str, perturb_ratio: float) -> str:
    assert model_name, "model_name must be non-empty"
    ratio_text = f"{perturb_ratio:g}".replace(".", "p")
    ...
    return f"{model_slug}_r{ratio_text}"
...
def build_output_dir(
    ...
    path = Path(output_dir) / experiment_name / "memory_pollution" / task / checkpoint_tag
